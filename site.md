...
- 资源目录：`resources.v3`
- 站点索引包：`user.sites.v3.bin`
- 索引版本：`3.0.0`

V3 不会回退到 V2 站点资源。遇到站点分类无法识别时，先确认运行实例已经加载 V3 资源，再检查 Cookie 和站点页面是否发生变化。

### 音乐分支

V3 除了音乐专用站点，也支持影视综合站中的音乐分支。当前索引已覆盖憨憨、观众、听听歌、春天、家园、天空、NovaHD、学校、麒麟、天雪、烧包乐园、自由农场、CARPT、红豆饭、冬樱、UBits、蟹黄堡、库非、PTLGS、AGSVPT、浦园 PT、彩虹岛、咖啡、下水道和我的 PT 等站点的音乐分类；实际可用站点以实例当前加载的资源和站点登录状态为准。

音乐搜索会使用站点索引中的 `music` 分类。对于使用父分类参数或独立搜索页面的站点，参数由 V3 索引自动生成，用户不需要手工拼接分类 URL。
...
**V3版本：** 站点优先级继续参与资源选择，同时音乐站点分类也会参与媒体类型过滤。搜索音乐时，只有站点索引声明了音乐分类或音乐专用类型的站点会返回音乐资源。

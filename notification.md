...
当智能助手已启用，且对应通知渠道已完成接入后，消息入口不再只是用来收通知，也可以作为智能助手的远程交互入口。

常见能力包括：

- 发送文本让智能助手分析搜索、下载、整理、订阅问题
- 在支持附件能力的渠道中发送图片，让智能助手辅助识别或判断
- 在支持附件能力的渠道中发送文件，并由系统保存原始附件，再把本地文件路径交给智能助手处理
- 在支持语音能力的渠道中发送语音，并先完成语音识别，再继续智能助手对话

关于智能助手本身的配置项、CLI 用法、图片输入开关等，参考 [智能助手](/agent)。
{.is-info}

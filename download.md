...
V3 音乐下载复用现有下载器和下载任务流程：在音乐搜索结果中选择站点资源后提交下载，下载完成后由文件整理流程识别音频并按音乐目录规则整理。

音乐下载不会创建 MoviePilot 内置音乐库。请先在 `设定 -> 目录` 配置音乐目标目录，并确认下载器路径与 MoviePilot 的目录映射一致；整理后的文件由外部媒体服务器负责扫描和播放。
...
如果同时已启用智能助手，则这些消息入口还可以继续承担智能助手远程交互能力，例如发送文本、图片、文件或语音消息进一步分析问题。参考 [通知](/notification) 与 [智能助手](/agent)。

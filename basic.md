...
# 用户初始密码
为了确保安全，MoviePilot 首次安装时需要为超级管理员设置初始密码。

- 如果安装时已设置 `SUPERUSER_PASSWORD`，或者在安装向导 / 本地 CLI 初始化向导中填写了管理员密码，则该值就是初始登录密码。
- 如果未设置，则系统会在第一次启动时生成随机复杂密码，**需要从第一次启动的后台日志文件或终端日志中获取管理员用户初始密码才能登录系统**。
...
首次安装向导默认使用 `themoviedb` 作为识别来源，以减少首次使用时因来源切换导致的识别偏差。后续如有需要，仍可在系统设置中调整相关搜索与识别来源。

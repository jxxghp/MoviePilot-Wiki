...
# 用户初始密码
V3 全新安装不会在启动阶段初始化管理员用户名或密码，也不会把初始密码打印到日志。首次访问 Web 地址时，系统会自动进入初始化页面；输入管理员用户名、密码和确认密码并保存后，才会创建管理员账号。

V1、V2 保持原有行为：

- 如果安装时已设置 `SUPERUSER_PASSWORD`，或者在 V2 设置向导 / 本地 CLI 初始化向导中填写了管理员密码，则该值就是初始登录密码。
- 如果未设置，则系统会按对应版本规则生成随机复杂密码，**需要从第一次启动的后台日志文件或终端日志中获取管理员用户初始密码才能登录系统**。

登录后可在 `设定 -> 用户` 中修改密码。

- V1、V2 的日志文件路径为`/config`对应映射目录下`logs/moviepilot.log`文件。
...
- V1、V2 首次安装时可直接在初始化向导里完成用户站点认证
- 进入系统后，点击右上角头像菜单中的 `用户认证`
- 使用环境变量配置 `AUTH_SITE` 及对应站点参数

推荐优先使用前两种方式，因为系统会直接按照当前资源支持情况动态列出可认证站点。环境变量方式仍然适用于自动化部署或批量维护场景，参考 [配置参考](/configuration) 中 `AUTH_SITE` 的说明。
...
V1、V2 首次安装向导默认使用 `themoviedb` 作为识别来源，以减少首次使用时因来源切换导致的识别偏差。V3 首次初始化只负责创建管理员和 API Key，识别来源等完整设置在登录后按需调整。

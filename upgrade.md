...
V3 是独立主版本，但兼容 V2 的配置和数据库数据；未使用 V3 变更合同的插件可直接复用，需要适配的插件应安装开发者提供的 V3 专用版本。从 V2 切换到 V3 不需要迁移用户数据，继续映射原 V2 的 `/config` 目录并复用原数据库即可；切换前请先完整备份。详见 [V3 版本说明](/v3) 和 [V3 插件适配指引](/plugin-v3-adaptation)。

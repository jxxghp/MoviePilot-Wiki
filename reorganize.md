...
V3 将音乐作为与电影、电视剧并列的媒体类型。下载完成后，系统会读取音频标签和文件技术参数，结合订阅或搜索时保存的音乐元数据，按音乐重命名格式整理到目标目录。
...
在 `设定 -> 目录 -> 音乐重命名格式` 中可使用 Jinja2 格式调整目录和文件名，常用字段包括 `artist`、`album_artist`、`album`、`title`、`track`、`disc`、`year` 和 `fileExt`。

MoviePilot 只负责下载后的识别、整理和刮削，不建立音乐媒体库、不播放音频，也不负责歌单或歌手库同步。整理完成后请由 Navidrome、Emby、Jellyfin、Plex 等媒体服务器扫描目标目录。

> 音乐整理会迁移与音轨同目录、同主干名的 `.lrc`、`.txt` 和 `.lyricsfile.yaml`。启用音乐刮削后可写入标签和封面，并按“质量升级”策略读取内嵌歌词、查询在线来源和生成旁挂歌词；已有同步歌词不会被纯文本降级覆盖。刮削会修改目标音频文件，硬链接模式下也会反映到同一底层文件。
{.is-info}
...
- 整理整张音乐专辑时，可在`文件管理`中对专辑目录直接点击整理，或进入多选模式勾选该目录；类型选择`音乐`后会默认使用`专辑`实体，单个音频文件则默认使用`单曲`实体。

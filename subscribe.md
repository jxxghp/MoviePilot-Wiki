...
V3 支持创建歌曲或专辑订阅。音乐订阅会复用现有订阅站点、订阅刷新、资源过滤、下载和整理流程；底层不新增一套音乐任务队列。

音乐订阅在订阅页面中单独显示，可从音乐搜索结果或音乐详情直接创建。订阅站点必须已经配置有效 Cookie，并且 V3 站点索引中存在音乐分类或音乐专用类型。

> 音乐订阅只负责自动检索、下载和整理，不负责歌单管理、歌手库维护或媒体服务器内的播放状态同步。
{.is-info}
